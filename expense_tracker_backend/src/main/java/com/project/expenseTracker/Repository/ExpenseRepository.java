...
public interface ExpenseRepository extends JpaRepository<MonthlyExpenses,Integer>{
    ...
    @Query(value = "SELECT left_over_amount FROM monthly_expenses where extract(month from created_date)=:month and created_date=(SELECT max(created_date) FROM monthly_expenses)", nativeQuery = true)
    public Optional<Double> findtheLastMonthlyExpense(@Param("month") Integer month);
    ...
    @Query(value = "SELECT extract(month from created_date) as Month from monthly_expenses me where email_id=:email limit 3",nativeQuery = true)
    ...
    @Query(value="SELECT price_of_expense from monthly_expenses me where me.id=:identifier",nativeQuery = true)
    public double getPriceOfProductToBeDeleted(@Param("identifier") Integer Id);
}
